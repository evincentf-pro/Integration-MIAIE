...
if (isset($_GET['delete_id'])) {
    ...
    $stmt_select = $DB_con->prepare('SELECT img FROM alaune_dgia WHERE id =:uid');
    $stmt_select->execute(array(':uid' => $_GET['delete_id']));
    $imgRow = $stmt_select->fetch(PDO::FETCH_ASSOC);
    unlink("images/" . $imgRow['image']);

    // it will delete an actual record from db
    $stmt_delete = $DB_con->prepare('DELETE FROM alaune_dgia WHERE id =:uid');
                        ...
                        <div class="card-body">
                            <table class="table">
                                ...
                                <tbody style="text-align:center">
                                    ...
                                    $stmt = $DB_con->prepare('SELECT id, appeltitre, titre, legende, chapeau, corps, auteur, datepubli, img FROM alaune_dgia ORDER BY id DESC');
                                    $stmt->execute();
